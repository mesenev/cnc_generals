...
public class SoundManager
{
	...
	public Sound PlaySound(
		string soundFile,
		string polyphonicGroup = null,
		float pan = 0,
		int polyphonicMinInterval = 50,
		float pitch = 1,
		float priority = 0.5f,
		float volume = 1.0f,
		bool looped = false
		...
		if (polyphonicGroup != null) {
			...
			if (
				effectTimestamps.TryGetValue(polyphonicGroup, out value)
		...
		var sound = AudioSystem.Play(
			"Audio/Sounds/" + path, AudioChannelGroup.Effects, looped, priority, 0, false, volume, pan
		);
		if (pitch != 1) {
			sound.Pitch = pitch;
		}
